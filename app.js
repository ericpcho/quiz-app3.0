...
function renderQuizApp() {
  ...
  if (STORE.pageState === "page3") {
    ...
    <p class="js-feedback-answer">Score: ${STORE.correctQuestions}/${(STORE.totalQuestions + 1)}</p>
    ...
    if (STORE.pageState === "page3.5") {
      ...
      <p class="js-feedback-text">Correct Answer: ${STORE.questions[i].answers[STORE.questions[i].correctAnswer]}</p>
      <p class="js-feedback-answer">Score: ${STORE.correctQuestions}/${(STORE.totalQuestions + 1)}</p><button type="submit" class="js-next-question">Get Results</button>`)
    }
...
//   while(STORE.questions.length <=5){
